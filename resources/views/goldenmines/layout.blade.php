...
@if(Auth::guest())
    ...
    @else
        <body data-user-id="{{Auth::user()->id}}"
              data-user-balance="@if(Auth::user()->deposit == 0 && Auth::user()->money == 0) {{Auth::user()->bonus_money}} @else {{Auth::user()->money}} @endif" @yield('body_bg')>
        @endif
        @if (!Auth::guest() && (float)Auth::user()->deposit <= 0)
            <div class="bonus-banner_auth">
                <div class="hidden-xs container-fluid">
                    <button data-toggle="bonus-modal"
                            class="bonus-banner__button modal-toggle"> {{ __('Read') }} </button>
                </div>
            </div>
        @endif
